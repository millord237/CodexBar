...
struct ClaudeUsageFetcher: Sendable {
    ...
    func loadLatestUsage(model: String = "sonnet") async throws -> ClaudeUsageSnapshot {
        ...
        guard
            ...
            let snapshot = Self.parse(json: data)
        else {
    ...
    // Minimal variant of the Agent Sessions probe script. Uses optional overrides:
    // CODEXBAR_CLAUDE_BIN, CODEXBAR_TMUX_BIN, CODEXBAR_CLAUDE_MODEL, CODEXBAR_WORKDIR, CODEXBAR_CLAUDE_TIMEOUT, CODEXBAR_CLAUDE_SLEEP_BOOT, CODEXBAR_CLAUDE_SLEEP_AFTER.
...
error_json() {
  local code="$1"; local hint="$2"
  echo "{\"ok\":false,\"error\":\"$code\",\"hint\":\"$hint\"}"
}

if ! command -v "$TMUX_BIN" >/dev/null 2>&1; then
  echo "$(error_json tmux_not_found 'Install tmux: brew install tmux')" ; exit 15
fi
if ! command -v "$CLAUDE_BIN" >/dev/null 2>&1; then
  echo "$(error_json claude_cli_not_found 'Install Claude CLI from https://docs.claude.com')" ; exit 14
fi
...
"$TMUX_BIN" -L "$LABEL" resize-pane -t "$SESSION:0.0" -x 120 -y 32 >/dev/null 2>&1 || true
...
while [ $iterations -lt $max_iterations ]; do
  ...
  output=$("$TMUX_BIN" -L "$LABEL" capture-pane -t "$SESSION:0.0" -p -J 2>/dev/null || true)
  lower=$(echo "$output" | tr '[:upper:]' '[:lower:]')
  if echo "$lower" | grep -q "do you trust the files in this folder"; then
    "$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" "1" Enter
    sleep 1
    continue
  fi
  if echo "$lower" | grep -q "select a workspace"; then
    "$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" Enter
    sleep 1
    continue
  fi
  if echo "$lower" | grep -q "telemetry" && echo "$lower" | grep -q "(y/n)"; then
    "$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" "n" Enter
    sleep 1
    continue
  fi
  if echo "$lower" | grep -qE '(sign in|login|please run.*claude login)'; then
    echo "$(error_json auth_required_or_cli_prompted_login 'Run: claude login')" ; exit 13
  fi
  if echo "$lower" | grep -qiE '(claude code|tab to toggle|try )'; then booted=true; break; fi
done
if [ "$booted" = false ]; then
  sample=$("$TMUX_BIN" -L "$LABEL" capture-pane -t "$SESSION:0.0" -p -S -120 -J 2>/dev/null | head -n 30 | tr '\n' ' ' | sed 's/\\s\\+/ /g')
  echo "$(error_json tui_failed_to_boot "TUI did not boot within ${TIMEOUT_SECS}s; sample: ${sample:0:400}")"; exit 12
fi

"$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" "/" >/dev/null 2>&1; sleep 0.2
"$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" "usage" >/dev/null 2>&1; sleep 0.3
"$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" Enter >/dev/null 2>&1
tries=0
usage_output=""
while [ $tries -lt 6 ]; do
  sleep "$SLEEP_AFTER_USAGE"
  "$TMUX_BIN" -L "$LABEL" send-keys -t "$SESSION:0.0" Tab Tab Tab >/dev/null 2>&1
  sleep 0.2
  usage_output=$("$TMUX_BIN" -L "$LABEL" capture-pane -t "$SESSION:0.0" -p -S -200 -J 2>/dev/null || true)
...
session_data=$(parse_block "Current session")
week_all_data=$(parse_block "Current week \\(all models\\)")
week_opus_data=$(parse_block "Current week \\(Opus\\)")

session_pct=${session_data%%|*}; session_reset=${session_data#*|}
week_all_pct=${week_all_data%%|*}; week_all_reset=${week_all_data#*|}

if [ -z "$session_pct" ] || [ -z "$week_all_pct" ]; then
  echo "$(error_json parsing_failed 'Failed to extract usage data from TUI')" ; exit 16
fi

if [ -n "$week_opus_data" ]; then
  opus_pct=${week_opus_data%%|*}; opus_reset=${week_opus_data#*|}
  opus_json="{\"pct_used\": $opus_pct, \"resets\": \"$opus_reset\"}"
else
  opus_json=null
fi
...
{
  "ok": true,
  "session_5h": { "pct_used": $session_pct, "resets": "$session_reset" },
  "week_all_models": { "pct_used": $week_all_pct, "resets": "$week_all_reset" },
  "week_opus": $opus_json
}
EOF
"""
